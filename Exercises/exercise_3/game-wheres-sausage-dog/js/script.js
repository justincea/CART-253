...
function setup() {
  ...
  //Projects and positions the ''missing dog'' poster.
    rect(750,50,200,200);
    image(targetImage,850,140,150,150);
    fill(50);
    textSize(22);
    text(s,756,210);
...
//Allows dog to move in a weird way when clicked
    targetX = targetX + targetVY;
...
function mousePressed() {
  ...
  if (mouseX > targetX - targetImage.width/2 && mouseX < targetX + targetImage.width/2) {
    ...
    if (mouseY > targetY - targetImage.height/2 && mouseY < targetY + targetImage.height/2) {
      gameOver = true;
      //Gives the Target (Sausage Dog) a random velocity when clicked (found)
      targetVX = random(-5 ,5);
      targetVY = random(-5 ,5);
    }
  }
