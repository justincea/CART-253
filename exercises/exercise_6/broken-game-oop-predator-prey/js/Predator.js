...
class Predator {
  ...
  move() {
    // Update position
    this.x += this.vx;//FIXED missing [+] or [-] sign to allow them to move
    this.y += this.vy;//FIXED missing [+] or [-] sign to allow them to move
    // Update health
  ...
  handleEating(prey) {
    ...
    if (d < this.radius + prey.radius) {
      ...
      if (prey.health < 0) {
        prey.reset();//FIXED (changed rest to REET)
      }
    }
  }
